...
CREATE TABLE `base_chat_data` (
                                  `record_id` VARCHAR ( 100 ) DEFAULT NULL COMMENT '对话记录的ID',
...
CREATE TABLE `base_meta_data` (
                                  `u_id` VARCHAR ( 100 ) DEFAULT NULL COMMENT '会员的唯一值',
...
CREATE TABLE `base_visitor_data` (
                                     `id` VARCHAR ( 100 ) DEFAULT NULL COMMENT '访客ID，身份描述+ID',
                                     ...
                                     `se` DEFAULT NULL COMMENT '访客来访的搜索引擎',
                                     `keyword` DEFAULT NULL COMMENT '访客来访的搜索关键字',
...
CREATE TABLE `base_route_data` (
                                   `id` BIGINT ( 20 ) DEFAULT NULL COMMENT '接入对话的路由ID,-1：web访客的默认路由 -2：微信对话的默认路由 -3：SDK对话的默认路由',
...
CREATE TABLE `server_chat_start` (
                                     ...
                                     `refer_page` VARCHAR ( 100 ) DEFAULT NULL COMMENT '访客来源的URL，如果是微信，就是公众号名',
                                     `first_time` datetime DEFAULT NULL COMMENT '进入网站的时间',
                                     `first_page` DEFAULT NULL COMMENT '落地页',
                                     `first_page_title` DEFAULT NULL COMMENT '落地页面标题',
                                     `last_chat_time` datetime DEFAULT NULL COMMENT '上次对话时间',
                                     `last_chat_staff_id` BIGINT ( 20 ) DEFAULT NULL COMMENT '上次对话客服ID',
                                     `last_chat_staff_log_name` VARCHAR ( 100 ) DEFAULT NULL COMMENT '上次对话客服账号',
                                     `last_chat_staff_name` VARCHAR ( 100 ) DEFAULT NULL COMMENT '上次对话客服姓名',
...
CREATE TABLE `server_chat_end` (
                                   ...
                                   `chat_total_time` DEFAULT NULL COMMENT '通话时长 单位：毫秒',
                                   `chat_wait_time` INT ( 11 ) DEFAULT NULL COMMENT '等待时长 单位：毫秒',
                                   `first_answer_time` DEFAULT NULL COMMENT '首次响应时长 单位：毫秒',
                                   `avg_answer_time` DEFAULT NULL COMMENT '平均响应时长 单位：毫秒',
                                   `visitor_send_msg` DEFAULT NULL COMMENT '访客发送消息条数',
                                   `staff_send_msg` DEFAULT NULL COMMENT '客服发送消息条数',
                                   `perfect_matchs` DEFAULT NULL COMMENT '完全匹配数（机器人对话）',
                                   `exact_matchs` DEFAULT NULL COMMENT '精准匹配数（机器人对话）',
                                   `similarity_matchs` DEFAULT NULL COMMENT '相似匹配数（机器人对话）',
                                   `mis_match_count` DEFAULT NULL COMMENT '未命中数（机器人对话）',
                                   `solved_count` DEFAULT NULL COMMENT '点击解决数（机器人对话）',
                                   `unsolved_count` DEFAULT NULL COMMENT '点击未解决数（机器人对话）',
                                   PRIMARY KEY ( `msg_id` )
...
CREATE TABLE `sever_visitor_enter` (
                                       ...
                                       `visit_times` VARCHAR ( 100 ) DEFAULT NULL COMMENT '当前访客此次是第几次来访',
                                       PRIMARY KEY ( `msg_id` )
...
CREATE TABLE `sever_visitor_track_page` (
                                            ...
                                            `tm` BIGINT ( 20 ) DEFAULT NULL COMMENT '时间戳',
                                            `visitor_id` VARCHAR ( 100 ) DEFAULT NULL COMMENT '会员的唯一值',
                                            `track_record_id` BIGINT ( 20 ) DEFAULT NULL COMMENR '此次浏览轨迹ID',
                                            PRIMARY KEY ( `msg_id` )
